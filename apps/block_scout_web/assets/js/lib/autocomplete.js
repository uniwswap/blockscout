...
const dataSrc = async (query, id) => {
  try {
    ...
    const source = await fetch(`/token-autocomplete?q=${query}`);
...
document.querySelector('#main-search-autocomplete') &&
  document
    .querySelector('#main-search-autocomplete')
    .addEventListener('selection', function (event) {
      selection(event);
    });
document.querySelector('#main-search-autocomplete-mobile') &&
  document
    .querySelector('#main-search-autocomplete-mobile')
    .addEventListener('selection', function (event) {
      selection(event);
    });
